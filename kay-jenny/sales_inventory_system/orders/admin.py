...
class RefundAdmin(admin.ModelAdmin):
    list_display = ['order', 'amount', 'approved_by', 'created_at']
    list_filter = ['created_at']
    search_fields = ['order__order_number', 'reason']
    readonly_fields = ['order', 'payment', 'amount', 'reason', 'approved_by', 'created_at']

    def has_add_permission(self, request):
        """Prevent adding refunds directly through admin"""
        return False

    def has_change_permission(self, request, obj=None):
        """Make refunds read-only in admin"""
        return False

    def get_queryset(self, request):
        """Optimize queryset with select_related for foreign keys"""
        return super().get_queryset(request).select_related('order', 'payment', 'approved_by')
