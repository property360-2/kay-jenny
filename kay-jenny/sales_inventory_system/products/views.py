...
def product_list(request):
    ...
    low_stock_products = [
        ...
        if p.calculated_stock < p.threshold and p.calculated_stock > 0
    ]
    total_count = products.count()

    # Pagination
    paginator = Paginator(
        products.order_by("category", "name"), 12
        ...
        try:
            ...
            if requires_bom:
                ...
                if not ingredients_data:
                    messages.error(
                        request,
                        "Manufactured products must have at least one ingredient in the recipe.",
                    )
                    return render(
                        request,
                        "products/form.html",
                ...
                try:
                    ...
                    if stock < 0:
                        messages.error(
                            request, "Simple products must have a stock quantity >= 0."
                        )
                        return render(
                            request,
                            "products/form.html",
                    ...
                    messages.error(
                        request, "Simple products must have a valid stock quantity."
                    )
                    return render(
                        request,
                        "products/form.html",
            ...
            with transaction.atomic():
                ...
                product_type = (
                    "Manufactured (with BOM)" if requires_bom else "Simple stock item"
                )

                success_msg = f'Product "{product.name}" created successfully!'
                if requires_bom:
                    success_msg += (
                        f" Recipe with {ingredients_count} ingredient(s) added."
                    )
...
def archived_products_list(request):
    ...
    archived_products = (
        Product.objects.filter(is_archived=True)
        .select_related("recipe")
        .prefetch_related("recipe__ingredients__ingredient")
    )
    ...
    for product in page_obj:
        product.archive_info = {"archived_by": "Unknown", "archived_at": "Unknown date"}
...
def ingredient_list(request):
    ...
    sort = request.GET.get("sort", "stock_asc").strip()
    page_number = request.GET.get("page", 1)
    ...
    order_by_field = sort_map.get(sort, "current_stock")
    ingredients = ingredients.order_by(order_by_field, "name")
...
def ingredient_create(request):
    ...
    if request.method == "POST":
        try:
            ...
            variance_allowance = Decimal(
                request.POST.get("variance_allowance", "10.00")
            )
    ...
    context = {"action": "Create", "units": ["g", "ml", "pcs"]}
...
def ingredient_edit(request, pk):
    ...
    if request.method == "POST":
        try:
            ...
            ingredient.current_stock = Decimal(
                request.POST.get("current_stock", "0.00")
            )
            ingredient.min_stock = Decimal(request.POST.get("min_stock", "10.00"))
            ingredient.variance_allowance = Decimal(
                request.POST.get("variance_allowance", "10.00")
            )
    ...
    context = {"ingredient": ingredient, "action": "Edit", "units": ["g", "ml", "pcs"]}
    return render(request, "products/ingredient_form.html", context)
    ...
    if request.method == "POST":
        try:
            with transaction.atomic():
                ...
                messages.success(
                    request, f'Recipe for "{product.name}" updated successfully!'
                )
...
def api_create_ingredient(request):
    ...
    try:
        ...
        if not unit:
            return JsonResponse(
                {"success": False, "message": "Unit of measurement is required"}
            )
        ...
        if Ingredient.objects.filter(name=name).exists():
            return JsonResponse(
                {"success": False, "message": f'Ingredient "{name}" already exists'}
            )
        ...
        return JsonResponse(
            {"success": False, "message": f"Error creating ingredient: {str(e)}"}
        )
...
def api_create_category(request):
    ...
    try:
        ...
        return JsonResponse(
            {"success": False, "message": f"Error creating category: {str(e)}"}
        )
...
def api_search_archives(request):
    ...
    results = {"products": [], "users": [], "orders": [], "query": query}
    ...
    if archive_type in ["all", "users"]:
        archived_users = (
            User.objects.filter(is_archived=True)
            .filter(
